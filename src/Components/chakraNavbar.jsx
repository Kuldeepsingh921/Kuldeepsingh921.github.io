...
import { contextapi } from '../Contextapi';
  export default function ChakraNavbar() {
    ...
    const handleClick=()=>{
      window.open(Kuldeep_Singh_Resume,"_blank","noreferrer")
    }
    React.useEffect(() => {
      AOS.init();
    }, [])
        ...
        <Flex
          ...
          <Flex  flex={{ base: 1 }} justify={{ base: 'left', md: 'space-between', }}>
            <Text
              ...
              <Link href="/"> <Image className={styles.logo} height={{base:"50px",md:"55px",lg:"55px"}} width={{base:"",md:"",lg:"55px"}} src={colorMode==="light"?"https://i.postimg.cc/kgVRHSfX/1.png":"https://i.postimg.cc/KvGHtGQ5/2.png"  } /> </Link>
            </Text>
  ...
  const DesktopNav = ({handleClick}) => {
    ...
    return (
      <Stack  display="flex" alignItems="right" direction={'row'} spacing={10}>
        ...
        <Link  _hover={{textDecoration: 'none'}}  href={Kuldeep_Singh_Resume} download="Kuldeep_Singh_Resume.pdf"><Box className={styles.navbutton} onClick={handleClick}>Resume</Box></Link>
      </Stack>
    );
  };
